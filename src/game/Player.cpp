...
void Player::DeleteFromDB(ObjectGuid playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    switch(charDelete_method)
    {
        ...
        case 0:
        {
            ...
            CharacterDatabase.PExecute("DELETE FROM `jail` WHERE `guid` = '%u'",lowguid);
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, float honor)
{
    ...
    if (uVictim != NULL)
    {
        ...
        honor *= 2.0f;                                      //increase honor for hk's by 100% as of 3.3.3
    }
...
void Player::ChangeGrantableLevels(uint8 increase)
{
    if (increase)
    {
        if (m_GrantableLevelsCount <= int32(uint32(sWorld.getConfig(CONFIG_UINT32_RAF_MAXGRANTLEVEL) * sWorld.getConfig(CONFIG_FLOAT_RATE_RAF_LEVELPERLEVEL))))
