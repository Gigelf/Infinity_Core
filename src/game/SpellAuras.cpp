...
void Aura::HandleAuraModStalked(bool apply, bool /*Real*/)
{
    ...
    // assassins mark fix
    if(m_removeMode == AURA_REMOVE_BY_DEFAULT && GetAuraDuration() <= 0 && GetSpellProto()->Id == 46459)
...
//                pCaster->ProcDamageAndSpell(target, procAttacker, procVictim, PROC_EX_NORMAL_HIT, pdamage, BASE_ATTACK, spellProto);
        ...
        case SPELL_AURA_PERIODIC_ENERGIZE:
        {
            ...
            if( m_modifier.m_auraname == SPELL_AURA_OBS_MOD_ENERGY )
                pdamage = uint32(target->GetMaxPower(powerType) * amount/100);
            else
                pdamage = amount;
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch(GetId())
            {
                ...
                case 69674:                                 // Mutated Infection
                {
                    if (!apply)
                    {
                        if (m_removeMode == AURA_REMOVE_BY_DISPEL || m_removeMode == AURA_REMOVE_BY_EXPIRE)
                        {
                            cast_at_remove = true;
                            spellId1 = 69706;
                        }
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            else
            {
                ...
                if (GetId() == 29131 && m_target->HasAura(70844))
                {
                    ...
                    m_target->CastCustomSpell(m_target, 70845, &bp, NULL, NULL, true);
                    return;
                }
            }
            break;
